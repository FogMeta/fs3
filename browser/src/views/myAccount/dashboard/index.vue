        ...
        <el-col :span="12" v-for="(item,index) in card" :key="index">
          <el-card class="box-card">
            ...
            <div class="statusStyle">
              <div class="list">
                <svg t="1638936551170" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3518" width="128" height="128">
                  <path d="M661.333333 170.666667l253.866667 34.133333-209.066667 209.066667zM362.666667 853.333333L108.8 819.2l209.066667-209.066667zM170.666667 362.666667L204.8 108.8l209.066667 209.066667z" fill="#f8b551" p-id="3519"></path>
                  <path d="M198.4 452.266667l-89.6 17.066666c-2.133333 14.933333-2.133333 27.733333-2.133333 42.666667 0 98.133333 34.133333 192 98.133333 264.533333l64-55.466666C219.733333 663.466667 192 588.8 192 512c0-19.2 2.133333-40.533333 6.4-59.733333zM512 106.666667c-115.2 0-217.6 49.066667-292.266667 125.866666l59.733334 59.733334C339.2 230.4 420.266667 192 512 192c19.2 0 40.533333 2.133333 59.733333 6.4l14.933334-83.2C563.2 108.8 537.6 106.666667 512 106.666667zM825.6 571.733333l89.6-17.066666c2.133333-14.933333 2.133333-27.733333 2.133333-42.666667 0-93.866667-32-185.6-91.733333-258.133333l-66.133333 53.333333c46.933333 57.6 72.533333 130.133333 72.533333 202.666667 0 21.333333-2.133333 42.666667-6.4 61.866666zM744.533333 731.733333C684.8 793.6 603.733333 832 512 832c-19.2 0-40.533333-2.133333-59.733333-6.4l-14.933334 83.2c25.6 4.266667 51.2 6.4 74.666667 6.4 115.2 0 217.6-49.066667 292.266667-125.866667l-59.733334-57.6z"
                    fill="#f8b551" p-id="3520"></path>
                  <path d="M853.333333 661.333333l-34.133333 253.866667-209.066667-209.066667z" fill="#f8b551" p-id="3521"></path>
                </svg>
                In process jobs: {{item.inProcessJobs}}
              </div>
              <div class="list">
                <span class="el-icon-success"></span> Completed jobs: {{item.completedJobs}}</div>
              <div class="list">
                <span class="el-icon-error"></span> Failed jobs: {{item.failedJobs}}</div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      axios.get(_this.data_api + `/minio/backup/stat`, {        headers: {
          'Authorization': "Bearer " + _this.$store.getters.accessToken
        }      }).then((response) => {
        let json = response.data
        if (json.status == 'success') {
          _this.card[0].inProcessJobs = json.data.running_cnt
          _this.card[0].completedJobs = json.data.completed_cnt
          _this.card[0].failedJobs = json.data.failed_cnt
        } else {
      ...
      axios.get(_this.data_api + `/minio/rebuild/stat`, {        headers: {
          'Authorization': "Bearer " + _this.$store.getters.accessToken
        }      }).then((response) => {
        let json = response.data
        if (json.status == 'success') {
          _this.card[1].inProcessJobs = json.data.running_cnt
          _this.card[1].completedJobs = json.data.completed_cnt
          _this.card[1].failedJobs = json.data.failed_cnt
        } else {
          _this.$message.error(json.message);
          return false
        }
