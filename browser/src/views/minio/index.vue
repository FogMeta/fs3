  ...
  <div class="landing" @click="actClient(0)">
    <header class="fe-header">
      ...
      <div class="feh-used">
        ...
        <ul>
          <li>
            <span>Used: </span>{{userd | formatbytes}}</li>
        </ul>
      </div>
      <ul class="feh-actions">
        <li>
          <div class="dropdown btn-group">
            ...
            <button type="button" class="btn btn-default mobileIcon" @click.stop="signBtn">
            ...
            <ul class="dropdown-menu" v-show="signShow">
              ...
              <li @click="openAbout">
                <a href="javascript:;" id="show-about">About
                  <i class="iconfont icon-i"></i>
                </a>
              </li>
              <li @click="logout">
                <router-link to="/fs3/login" id="logout">Sign Out
    ...
    <div class="table">
      <el-table :data="tableData" stripe style="width: 100%" :row-key="getRowKeys" :expand-row-keys="expands" @expand-change="exChange">
        ...
        <el-table-column prop="size" label="Size" sortable width="130">
          <template slot-scope="scope">
        ...
        <el-table-column prop="lastModified" sortable label="Last Modified" :formatter="formatter" width="180">
        </el-table-column>
    ...
    <el-drawer :visible.sync="drawer" :direction="direction" :modal="false" :modal-append-to-body="false" :before-close="handleClose" class="drawStyle01">
      <div class="draw_cont">
        <div class="draw_left">
          <i class="el-icon-success"></i> {{drawIndex}} Object selected</div>
    ...
    <!-- delete dialog box -->
    <el-dialog :visible.sync="deleteDialogVisible" custom-class="deleteStyle" :show-close="false" center>
      <img :src="danger_img" />
    ...
    <!-- share dialog box -->
    <share-dialog v-if="isRouterAlive" :shareDialog="shareDialog" :shareObjectShow="shareObjectShow" :shareFileShow="shareFileShow" :num="num" :share_input="share_input" :postAdress="postAdress" :sendApi="sendApi" @getshareDialog="getshareDialog"
      @getShareGet="getPresignedGet">
    </share-dialog>

    <!-- change password dialog box -->
    <change-password :changePass="changePass" @getChangePass="getChangePass">
    </change-password>

    <!-- import dialog -->
    <el-dialog title="Import" :visible.sync="dialogImportVisible" custom-class="importStyle" @close="resetForm('ruleForm')">
      <el-form :model="ruleForm" label-width="160px" :rules="rules" ref="ruleForm" class="demo-ruleForm" v-loading="loading">
        <el-form-item label="Endpoint:" prop="endpoint">
          <el-input v-model="ruleForm.endpoint"></el-input>
        </el-form-item>
        <el-form-item label="Access Key ID:" prop="accessKeyID">
          <el-input v-model="ruleForm.accessKeyID"></el-input>
        </el-form-item>
        <el-form-item label="Secret Access Key:" prop="secretAccessKey">
          <el-input v-model="ruleForm.secretAccessKey"></el-input>
        </el-form-item>
        <el-form-item label="Bucket Name:" prop="name">
    ...
    submitForm (formName) {
      ...
      _this.$refs[formName].validate((valid) => {
        if (valid) {
          _this.loading = true
          let minioDeal = {
            "endpoint": _this.ruleForm.endpoint,
            "access_key_id": _this.ruleForm.accessKeyID,
            "secret_access_key": _this.ruleForm.secretAccessKey,
            "bucket_name": _this.ruleForm.name,
            "location": _this.ruleForm.location,
            "target_bucket": _this.currentBucket
          }

          let postUrl = _this.data_api + `/minio/bucket/import/s3`
...
.landing {
  ...
  .fe-header {
    position: relative;
    padding: 0.4rem 0.4rem 0.2rem 0.45rem;
    .form_top {
      display: flex;
      align-items: center;
      flex-wrap: wrap;
  ...
  .import-style {
    padding: 0.2rem 0.2rem 0.1rem 0.45rem;
    font-size: 0.15rem;
